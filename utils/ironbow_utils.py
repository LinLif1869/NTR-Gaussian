...
def _build_lut():
    packed = np.asarray([int(value, 16) for value in _IRONBOW_HEX.split()], dtype=np.uint32)
    return np.stack(
        [
            (packed >> 16) & 0xFF,
            (packed >> 8) & 0xFF,
...
def _nearest_ironbow_indices(rgb):
    rgb = np.asarray(rgb, dtype=np.uint8)
    packed = _pack_rgb(rgb)
    unique, inverse = np.unique(packed.reshape(-1), return_inverse=True)

    missing = [int(value) for value in unique if int(value) not in _RGB_TO_INDEX_CACHE]
    palette = IRONBOW_LUT.astype(np.int32)
    for start in range(0, len(missing), 8192):
        chunk = np.asarray(missing[start:start + 8192], dtype=np.uint32)
        colors = _unpack_rgb(chunk)
        distances = ((colors[:, None, :] - palette[None, :, :]) ** 2).sum(axis=2)
        nearest = distances.argmin(axis=1)
        for value, index in zip(chunk, nearest):
            _RGB_TO_INDEX_CACHE[int(value)] = int(index)

    indices = np.asarray([_RGB_TO_INDEX_CACHE[int(value)] for value in unique], dtype=np.int32)
    return indices[inverse].reshape(packed.shape)


def ironbow_to_gray_rgb_pil(image):
    rgb = np.asarray(image.convert("RGB"), dtype=np.uint8)
    indices = _nearest_ironbow_indices(rgb)
    gray = np.rint(indices * 255.0 / max(len(IRONBOW_LUT) - 1, 1)).astype(np.uint8)
    gray_rgb = np.repeat(gray[..., None], 3, axis=2)
    return Image.fromarray(gray_rgb)


def gray_rgb_to_ironbow_pil(image):
    rgb = np.asarray(image.convert("RGB"), dtype=np.float32)
    gray = rgb.mean(axis=2) / 255.0
    indices = np.rint(np.clip(gray, 0.0, 1.0) * (len(IRONBOW_LUT) - 1)).astype(np.int32)
    return Image.fromarray(IRONBOW_LUT[indices])


def gray_to_ironbow_tensor(image):
    import torch

    gray = image[:3].mean(dim=0)
    indices = torch.round(torch.clamp(gray, 0.0, 1.0) * (len(IRONBOW_LUT) - 1)).long()
    lut = torch.as_tensor(IRONBOW_LUT, dtype=image.dtype, device=image.device) / 255.0
    return lut[indices].permute(2, 0, 1)
